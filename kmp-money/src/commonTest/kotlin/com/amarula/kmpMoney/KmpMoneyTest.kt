...
class KmpMoneyTest {
    ...
    fun `round with ROUND_HALF_AWAY_FROM_ZERO rounds 0_5 up for positive`() {
        assertEquals(
            "10.56",
            KmpMoney.of("10.555", Currency.USD)
                .round(RoundingMode.ROUND_HALF_AWAY_FROM_ZERO)
                .numberStrippedString
        )
    }
    ...
    fun `round with TOWARDS_ZERO truncates`() {
        assertEquals(
            "10.55",
            KmpMoney.of("10.559", Currency.USD)
                .round(RoundingMode.TOWARDS_ZERO)
                .numberStrippedString
        )
    }
    ...
    @Test
    fun `isLessThanOrEqualTo true when less`() {
        assertTrue(
            KmpMoney.of("1.00", Currency.USD).isLessThanOrEqualTo(KmpMoney.of("2.00", Currency.USD))
        )
    }

    @Test
    fun `isLessThanOrEqualTo true for equal amounts`() {
        assertTrue(
            KmpMoney.of("1.00", Currency.USD).isLessThanOrEqualTo(KmpMoney.of("1.00", Currency.USD))
        )
    }

    @Test
    fun `isLessThanOrEqualTo false when greater`() {
        assertFalse(
            KmpMoney.of("2.00", Currency.USD).isLessThanOrEqualTo(KmpMoney.of("1.00", Currency.USD))
        )
    }

    @Test
    fun `isLessThanOrEqualTo throws on currency mismatch`() {
        assertFailsWith<IllegalArgumentException> {
            KmpMoney.of("1.00", Currency.USD).isLessThanOrEqualTo(KmpMoney.of("2.00", Currency.EUR))
        }
    }
    ...
    fun `coerceAtLeast returns self when equal to min`() {
        ...
        assertEquals(
            "1.00",
            m.coerceAtLeast(KmpMoney.of("1.00", Currency.USD)).numberStrippedString
        )
    }
    ...
    fun `coerceAtMost returns self when below max`() {
        ...
        assertEquals(
            "5.00",
            m.coerceAtMost(KmpMoney.of("10.00", Currency.USD)).numberStrippedString
        )
    }
